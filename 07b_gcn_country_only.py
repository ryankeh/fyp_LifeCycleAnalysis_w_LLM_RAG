"""
Script 11a: GCN Only (No Industry Variables) for Carbon Intensity Prediction
Ablation study: Removes industry encoder, uses only GCN for country features
Uses: train.csv, val.csv, test.csv, country_distance_matrix_distwces.csv
Saves: gcn_only_results.json, predictions, visualizations
"""
...
print("=" * 60)
print("GCN ONLY (NO INDUSTRY VARIABLES) - ABLATION STUDY")
print("=" * 60)
...
# ============================================================================
# ABLATION 1: GCN Only Model (No Industry Variables)
# ============================================================================

class GCNAblationPredictor(nn.Module):
    """
    Ablation model: Only GCN for countries, no industry variables.
    Uses only country embeddings to predict carbon intensity.
    """
    def __init__(self, num_countries, country_input_dim, 
                 country_hidden_dim=64, country_output_dim=32,
                 final_hidden_dim=64, dropout=0.2):
        super(GCNAblationPredictor, self).__init__()
        
        # GCN for countries (same as original)
        self.gcn = GCN(
        ...
        # Final prediction network (takes only country embeddings)
        # To match original total parameters, we increase hidden size
        # Original: country(32) + industry(32) = 64 input
        # Now: only country(32) input, so we make final network wider
        self.predictor = nn.Sequential(
            nn.Linear(country_output_dim, final_hidden_dim * 2),  # Wider to compensate
            nn.ReLU(),
            nn.Dropout(dropout),
    ...
    def forward(self, country_idx, industry_idx=None):
        """
        Args:
            country_idx: [batch_size] indices of countries
            industry_idx: ignored (kept for API compatibility)
        """
        # Get country embeddings (pre-computed)
        country_emb = self.country_embeddings[country_idx]  # [batch × 32]
        
        # Predict directly from country embeddings
        output = self.predictor(country_emb)
...
# Country features only (no industry variables)
...
# Clip negative values from numerical precision
...
# Create data loaders (industry_idx is kept but ignored by model)
batch_size = 256
...
# ============================================================================
# INITIALIZE MODEL (GCN Only)
# ============================================================================

print("\n" + "-" * 40)
print("INITIALIZING GCN-ONLY MODEL")
print("-" * 40)

model = GCNAblationPredictor(
...
for epoch in range(num_epochs):
    ...
    for batch_country, batch_industry, batch_target in train_loader:
        ...
        optimizer.zero_grad()
        predictions = model(batch_country)  # Industry idx ignored
    ...
    with torch.no_grad():
        for batch_country, batch_industry, batch_target in val_loader:
            ...
            predictions = model(batch_country)
            loss = criterion(predictions, batch_target)
...
# Save scalers
joblib.dump(country_scaler, '../models/gcn_only_country_scaler.pkl')
print("Country scaler saved to: ../models/gcn_only_country_scaler.pkl")
...
test_results_df.to_csv('../results/gcn_only_test_predictions.csv', index=False)
print("Test predictions saved to: ../results/gcn_only_test_predictions.csv")
...
train_results_df.to_csv('../results/gcn_only_train_predictions.csv', index=False)
print("Train predictions saved to: ../results/gcn_only_train_predictions.csv")
...
val_results_df.to_csv('../results/gcn_only_val_predictions.csv', index=False)
print("Validation predictions saved to: ../results/gcn_only_val_predictions.csv")
...
summary_stats.to_csv('../results/gcn_only_summary_stats.csv', index=False)
print("Summary statistics saved to: ../results/gcn_only_summary_stats.csv")
...
axes[0, 0].set_title(f'GCN Only: Predicted vs Actual (R² = {test_metrics["R2"]:.4f})')
axes[0, 0].legend()
...
axes[0, 1].set_title('GCN Only: Residual Distribution')
axes[0, 1].legend()
...
axes[0, 2].set_title('GCN Only: Training History')
axes[0, 2].legend()
...
axes[1, 0].set_title('GCN Only: Absolute Error Distribution')
axes[1, 0].legend()
...
axes[1, 1].set_title('GCN Only: Metrics Across Datasets')
...
axes[1, 2].set_title('GCN Only: Log Scale Comparison')
axes[1, 2].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('../results/plots/gcn_only_results.png', dpi=150, bbox_inches='tight')
plt.show()
...
with open('../results/gcn_only_results.json', 'w') as f:
    json.dump(all_metrics, f, indent=2, default=str)

print("All metrics saved to: ../results/gcn_only_results.json")
...
print("\n" + "=" * 60)
print("GCN ONLY - ABLATION STUDY - FINAL SUMMARY")
print("=" * 60)

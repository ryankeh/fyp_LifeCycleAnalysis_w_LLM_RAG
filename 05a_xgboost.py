"""
Script 6: XGBoost for Carbon Intensity Prediction
Uses: train.csv, val.csv, test.csv
Saves: xgboost_results.json, xgboost_predictions.csv, visualizations
"""
...
print("=" * 60)
print("XGBOOST REGRESSION SCRIPT")
print("=" * 60)
...
# Prepare data (XGBoost works well with numpy arrays)
...
print(f"Total combinations: {len(param_grid['n_estimators']) * len(param_grid['max_depth']) * 
      len(param_grid['learning_rate']) * len(param_grid['subsample']) * len(param_grid['colsample_bytree'])}")

# Full grid search - iterate through ALL combinations
for n_est in param_grid['n_estimators']:           # 3 values
    for max_d in param_grid['max_depth']:          # 4 values
        for lr in param_grid['learning_rate']:     # 3 values
            for subsample in param_grid['subsample']:   # 2 values
                for colsample in param_grid['colsample_bytree']:  # 2 values
                    
                    # Train model - FIX: Remove early_stopping_rounds from fit method
                    ...
                    # FIX: Use eval_set correctly without early_stopping_rounds parameter
                    model.fit(
                        X_train, y_train,
                    ...
                    print(f"  n_est={n_est:3d}, depth={max_d}, lr={lr:.3f}, "
                          f"sub={subsample:.1f}, col={colsample:.1f} -> RMSE: {val_rmse:.6f}")
...
tuning_df.to_csv('../results/xgb_tuning_results.csv', index=False)
print("Tuning results saved to: ../results/xgb_tuning_results.csv")
...
# FIX: Include early_stopping_rounds in the constructor for final model too
...
# Train with early stopping - FIX: Remove early_stopping_rounds from fit
final_model.fit(
    X_train, y_train,
    eval_set=[(X_val, y_val)],
    verbose=50
...
# Save model
joblib.dump(final_model, '../models/xgboost_best_model.pkl')
...
feature_importance_df.to_csv('../results/xgb_feature_importance.csv', index=False)
...
test_results_df.to_csv('../results/xgb_test_predictions.csv', index=False)
print("Test predictions saved to: ../results/xgb_test_predictions.csv")
...
train_results_df.to_csv('../results/xgb_train_predictions.csv', index=False)
print("Train predictions saved to: ../results/xgb_train_predictions.csv")
...
val_results_df.to_csv('../results/xgb_val_predictions.csv', index=False)
print("Validation predictions saved to: ../results/xgb_val_predictions.csv")
...
summary_stats.to_csv('../results/xgb_summary_stats.csv', index=False)
print("Summary statistics saved to: ../results/xgb_summary_stats.csv")
...
if industry_metrics:
    ...
    industry_df.to_csv('../results/xgb_industry_metrics.csv', index=False)
    print(f"Industry metrics saved to: ../results/xgb_industry_metrics.csv")
    ...
    plt.tight_layout()
    plt.savefig('../results/plots/xgb_industry_performance.png', dpi=150, bbox_inches='tight')
    plt.show()
...
if country_metrics:
    ...
    country_df.to_csv('../results/xgb_country_metrics.csv', index=False)
    print(f"Country metrics saved to: ../results/xgb_country_metrics.csv")
    ...
    plt.tight_layout()
    plt.savefig('../results/plots/xgb_country_performance.png', dpi=150, bbox_inches='tight')
    plt.show()
    ...
    ax.set_title('XGBoost: Distribution of Country-Level RMSE')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('../results/plots/xgb_country_rmse_distribution.png', dpi=150, bbox_inches='tight')
    plt.show()
...
axes[0, 0].set_title(f'XGBoost: Predicted vs Actual (R² = {test_metrics["R2"]:.4f})')
axes[0, 0].legend()
...
axes[0, 2].set_title('XGBoost: Top 10 Feature Importances')
...
axes[1, 0].set_title('XGBoost: Absolute Error Distribution')
axes[1, 0].legend()
...
axes[1, 1].set_title('XGBoost: Metrics Across Datasets')
...
axes[1, 2].set_title('XGBoost: Log Scale Comparison')
axes[1, 2].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('../results/plots/xgboost_results.png', dpi=150, bbox_inches='tight')
plt.show()
...
print("All metrics saved to: ../results/xgboost_results.json")

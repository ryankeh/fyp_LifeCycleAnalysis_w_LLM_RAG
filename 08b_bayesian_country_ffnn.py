"""
Script B5: Bayesian Neural Network for Country Variables
Trains a Bayesian FFNN on the 8 country WDI variables
...
print("=" * 60)
print("BAYESIAN NEURAL NETWORK - COUNTRY MODEL")
print("=" * 60)
...
# ============================================================================
# BAYESIAN NEURAL NETWORK MODEL FOR COUNTRY VARIABLES
# ============================================================================

class BayesianCountryNet(nn.Module):
    def __init__(self, input_dim=8, hidden_dims=[32, 16]):
        super(BayesianCountryNet, self).__init__()
...
# Define country variables (8 WDI indicators)
country_vars = [
    'gdp_per_capita_ppp',
    ...
    'natural_resources_rents_pct'
]

# Prepare data
X_train = train_df[country_vars].values.astype(np.float32)
y_train = train_df['carbon_intensity'].values.astype(np.float32)
X_val = val_df[country_vars].values.astype(np.float32)
y_val = val_df['carbon_intensity'].values.astype(np.float32)
X_test = test_df[country_vars].values.astype(np.float32)
...
print("\n" + "-" * 40)
print("TRAINING BAYESIAN COUNTRY NN")
print("-" * 40)

model = BayesianCountryNet(input_dim=8, hidden_dims=[32, 16]).to(device)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        patience_counter = 0
        torch.save(model.state_dict(), '../models/bayesian/country_bnn.pt')
    else:
        patience_counter += 1
...
# Load best model
model.load_state_dict(torch.load('../models/bayesian/country_bnn.pt'))
...
# Save scalers
joblib.dump(scaler_X, '../models/bayesian/country_scaler_X.pkl')
joblib.dump(scaler_y, '../models/bayesian/country_scaler_y.pkl')
...
for split_name, split_df, predictions, stds in [
    ...
    results_df = pd.DataFrame({
        ...
        'country_pred': predictions,
        'country_std': stds,
        'split': split_name
    })
    results_df.to_csv(f'../results/bayesian/country_predictions_{split_name}.csv', index=False)
...
with open('../results/bayesian/country_metrics.json', 'w') as f:
...
axes[0].set_title('Training History - Country BNN')
axes[0].legend()
...
axes[1].set_title('KL Divergence - Country BNN')
axes[1].grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('../results/plots/bayesian/country_training.png', dpi=150)
plt.show()

print("\n" + "=" * 60)
print("COUNTRY BNN COMPLETE")
